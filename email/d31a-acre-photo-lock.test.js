...
must(/vauto:"\/docs\/vauto\.svg"/, "VAUTO uses the logo, not a GR walk-around");
mustNot(/vauto:"\/start-bgs\/gr-corolla/, "VAUTO does not use GR walk-around thumbs");
must(/isPhotoLockItem\(item\)\) return false/, "photo-lock never 90-day archives");
...
must(/id="exitTradeLanes"/, "Trade-in Sent/Waiting/History sit in quiet chrome");
must(/id="exitTradeSent">Sent/, "Trade-in Sent is a quiet chrome link");
must(/#inviteHistBtn, #usersHistBtn, #webdeskHistBtn, #centerSheetHistBtn, #photosHistBtn \{ display:none/, "loud header HISTORY capsules stay hidden");
must(/#tradeLanes, #webLanes \{ display:none/, "colored Sent/Waiting/History pills stay hidden");
