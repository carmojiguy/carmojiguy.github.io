...
must(/#startAppraise \{ top:auto/, "mobile Appraise sits in the dock action card");
must(/#startWebsite \{ bottom:auto/, "mobile Website sits in the dock action card");
...
must(/No appraisals in Incoming/, "empty desk short line");
...
must(/function startWebsitePost\(/, "start Website uses original home + story");
must(/function openWebdesk\(/, "website History desk stays");
...
must(/id="buildStamp">build d31a</, "home footer has a visible build stamp");
must(/id="typeSheet"[\s\S]*build d31a/, "type sheet hint includes the build stamp");
...
must(/rgba\(247,248,250/, "Center dim is a Snow Signal snow wash, not a dark trap");
...
must(/Walk the car and talk/, "CA story is walk-and-talk, not verbal-only");
mustNot(/Verbal description only|verbal description only/, "Verbal description only copy is gone");
