...
assert.ok(/if\(\(sent && sent\.ok\) \|\| texted\)\{\s*thankYouOnly\(\);/.test(invSrc), "email or SMS success never restoreSend");
assert.ok(/if\(!email\)\{\s*if\(texted\) thankYouOnly\(\);/.test(invSrc), "phone-only success never restoreSend");
assert.ok(/sendInviteSms\(/.test(invSrc), "invite SMS posts to the silent mailer");
assert.ok(!/fireSms\(/.test(invSrc), "invite Send does not open the Messages composer");
assert.ok(/APP\._thanksKind="invite"/.test(invSrc), "invite Thank you is the sender copy");
